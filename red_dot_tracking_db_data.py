def draw_skeleton_test(img_color, var_joints_recorded_data, var_joints_live_data, counter):
    ...
    ## PAUSE TEST ##
    ## -------------------------------------------- ##
    # RECORDED DATA - CHANGED TO LEFT_HAND FOR TESTING
    data_recorded_head = pd.DataFrame(var_joints_recorded_data['left_hand_df'])

    data_recorded = data_recorded_head

    # LIVE DATA
    data_live_head = np.array(var_joints_live_data.get('left_hand'))
